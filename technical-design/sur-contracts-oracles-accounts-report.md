...
- **نکته حیاتی:** در همین سورس، مقدار اولیه‌ی `distributionOracle` برابر `address(0)` است (یک 🔶 FILL_IN placeholder) — یعنی تا وقتی ابزار ساخت genesis این مقدار را واقعی نکند، هیچ‌کس نمی‌تواند توزیع پاداش را فراخوانی کند.
...
- ✅ **تفاوت مهم با سه اوراکل دیگر (رفع تناقض این سند — بخش ۶ را ببین):** برخلاف `distributionOracle`/`verifier`/`identityOracle`، مقدار اولیه‌ی `paymentOracle` هرگز `address(0)` در سورس نمی‌نشیند — چون `SurenSale` (تنها قرارداد غیر-genesis) یک `constructor(address _paymentOracle)` واقعی دارد که صراحتاً `require(_paymentOracle != address(0))` می‌کند. یعنی دیپلوی این قرارداد با آدرس صفر از همان ابتدا و روی زنجیره fail می‌شود — این با سه اوراکل دیگر که واقعاً به‌صورت placeholder `0x0` در سورس genesis می‌نشینند تا ابزار genesis پرشان کند، فرق بنیادین دارد.
...
- ✅ **اصلاح مهم — تناقض رفع‌شده درباره‌ی `paymentOracle`:** برخلاف عبارت‌بندی نسخه‌ی قبلی این سند، `paymentOracle` جزو «placeholder صفر در سورس» نیست — این کلید تنها از طریق آرگومان واقعی `constructor` قرارداد `SurenSale` (که اصلاً genesis-injected نیست) تنظیم می‌شود، و آن constructor صراحتاً آدرس صفر را رد می‌کند (`require`). سه اوراکل دیگر (`distributionOracle`، `verifier`، `identityOracle`) واقعاً به‌صورت `address(0)` در سورسِ قراردادهای genesis می‌نشینند و توسط ابزار genesis باید جایگزین شوند — این دو مکانیزم از نظر فنی کاملاً متفاوتند، نباید یک‌جا دسته‌بندی شوند (بند آخر همین بخش را هم ببین که این تفاوت را درست توضیح داده بود).
...
- **⚠️ هشدار عملیاتی مهم:** در همین نسخه از سورس، آدرس اولیه‌ی سه اوراکل genesis-injected (`distributionOracle`، `verifier`، `identityOracle`) و بسیاری از پارامترهای امنیتی ValidatorsRegistry (`probationPeriod`، `slashBps`، `exitCooldown` و…) هنوز placeholder صفر/`0x0` هستند (علامت‌گذاری‌شده با 🔶 FILL_IN) — پیش از استقرار واقعی باید با مقادیر نهایی جایگزین شوند. (`paymentOracle` جزو این فهرست نیست — بند بالا را ببین.)
...
- SurenSale تنها قراردادی است که constructor واقعی دارد و بعد از genesis، با یک تراکنش معمولی دیپلوی می‌شود؛ به همین دلیل `paymentOracle` اولیه به‌صورت آرگومان سازنده تنظیم می‌شود، نه placeholder.
