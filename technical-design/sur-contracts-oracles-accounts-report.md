...
- ✅ **وضعیت به‌روزشده:** آدرس اولیه‌ی هر چهار اوراکل دیگر placeholder نیست (بند بالا را ببین) — پارامترهای امنیتی دیگر `ValidatorsRegistry` (`probationPeriod=۱هفته`، `slashBps=۱٪`، `exitCooldown=۱هفته` و بقیه) هم همه نهایی شدند (جزئیات کامل در `sur-tokenomics.md` بخش ۶).
