...
این سند فقط شش قرارداد ساختاری genesis + `SurenSale` را پوشش می‌دهد. یک قرارداد دیگر — **`SurZether.sol`** (تراکنش‌های محرمانه، پیشنهادی) — در `contracts/reference-dapps/` قرار دارد و **عمداً بیرون از این هشت قرارداد** نگه داشته شده: constructor معمولی دارد، آدرس ثابت genesis ندارد، و مثل هر دپ دیگری دیپلوی می‌شود، نه به‌عنوان زیرساخت اصلی شبکه. جزئیات کامل در `sur-zether-confidential-transfers-proposal.md`.
