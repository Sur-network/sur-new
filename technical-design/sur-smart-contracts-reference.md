...
- **کارمزد عضویت (membership fee)** = `currentMembershipFee()` (درصدی از وثیقه، پارامتر `membershipFeeBps`): ✅ **به‌روزشده:** همان لحظه‌ی ورود، با `.call{value: ...}`، به `BlockRewardDistributor.receiveMembershipFee()` فوروارد می‌شود (نه مستقیم به `ValidatorsTreasury`) — جایی که در epoch توزیع بعدی، ۱۰۰٪-به‌نسبت-بلاک بین ولیدیتورهای فعال تقسیم می‌شود. ⚠️ **دقت رونداری:** «۱۰۰٪» یک توصیف اقتصادی درسته، نه یه ثابت ریاضی دقیق در سطح wei — چون تقسیم به نسبت بلاک از تقسیم صحیح عددی استفاده می‌کنه، حداکثر `تعداد ولیدیتور فعال − ۱` wei باقیمانده (`feeDust`) به‌عنوان خرده‌ریز به خزانه می‌ره، نه به ولیدیتورها. این مبلغ از نظر اقتصادی کاملاً ناچیزه (کمتر از یک wei به‌ازای هر ولیدیتور)، پس این تصمیم گرفته شد که به‌جای پیچیده‌کردن قرارداد با carry-forward این خرده‌ریز به epoch بعد، همون‌جا به خزانه بره — دقیقاً مثل رفتار خرده‌ریز فی معمولی و ریوارد. این یعنی ورود هر ولیدیتور جدید، یک انگیزه‌ی نقدی مستقیم و قابل‌ردیابی به ولیدیتورهای موجود می‌دهد، بدون این‌که خودِ وثیقه از مالکیت شخصی خارج شود.
...
|---|---|---|
| `entryThresholdBase` | `500,000 سورن` (`500_000 ether` در کد) | ✅ نهایی‌شده — کاهش از ۲ میلیون اولیه؛ با فرض قیمت اولیه‌ی سورن ≈ ۱۰۰ تومان (۰.۰۰۰۵ دلار)، یعنی ≈ ۲۵۰ دلار وثیقه برای صندلی اول |
| `growthFactorPerValidator` | `1.017480` (`1_017479692102686336` به fixed-point ۱۸ رقمی) | ✅ نهایی‌شده — دوره‌ی دوبرابرشدن از ۱۶ به ۴۰ ولیدیتور *پرداخت‌کننده* افزایش یافت (۷ ولیدیتور مؤسس رایگان شمرده نمی‌شوند) |
...
- از `totalRewards`: ۱۵٪ ثابت (`FOUNDATION_SHARE_BPS`) به `FoundationDAO`؛ `validatorDirectShareBps` (حکمرانی‌شونده، شروع ۵۰٪) به نسبت `blocksMined` مستقیم بین ولیدیتورها؛ باقی‌مانده به `TREASURY`.
...
✅ **تغییر مهم در `BlockRewardDistributor.sol` و `ValidatorsRegistry.sol` (به‌روزرسانی این نسخه):** مدل توزیع دیگر دومقصدی نیست — اکنون سه‌مقصدی است (ولیدیتورها، خزانه، و بنیاد)، با ۱۵٪ ثابت بنیاد از **کل** بلاک‌ریوارد (نه از سهم خزانه — کاملاً مستقل و بدون‌ارتباط با هر تغییر آینده در نسبت خزانه/ولیدیتور)؛ کارمزد عضویت هم دیگر به خزانه نمی‌رود، بلکه از طریق `receiveMembershipFee()` به `BlockRewardDistributor` فوروارد و در epoch توزیع بعدی، ۱۰۰٪-به‌نسبت-بلاک بین ولیدیتورهای فعال تقسیم می‌شود.
