...
contract ValidatorsRegistry {
    ...
    /// @notice ✅ تصمیم قطعی: حداقل **کسری** از چک‌هایی که باید واقعاً ثبت شده باشه قبل از
    ///         این‌که اصلاً شرط نسبت ارزیابی بشه. بر مبنای EXPECTED_VERIFIER_CADENCE_SECONDS
    ///         پایین (چرخه‌ی واقعی و تصمیم‌گرفته‌شده‌ی Verifier — ۱۵ دقیقه) — عمداً *نه*
    ///         MIN_LIVENESS_CHECK_INTERVAL (throttle ضدتکرار جداگانه‌ی ۱۳دقیقه‌ای درست پایین):
    ...
    ///         چک مثبت تنها — که هر لحظه، حتی درست انتهای پنجره برسه — نسبت ۱۰۰٪ می‌ساخت و
    ///         requiredLivenessRatioBps رو با صفر تاریخچه‌ی پایش واقعی برآورده می‌کرد.
