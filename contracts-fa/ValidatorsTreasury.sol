...
///           - ✅ به‌روزشده: هرچه از بلاک‌ریوارد باقی بماند بعد از کسر سهم ثابت ۱۵٪ بنیاد
///             (از کل، مستقل از این) و سهم مستقیم و حکمرانی‌شونده‌ی ولیدیتورها
///             (validatorDirectShareBps، بازه‌ی [۴۰٪, ۶۵٪] از کل، قابل‌تغییر با رأی‌گیری
///             دومجلسی در BlockRewardDistributor.sol) — به sur-tokenomics.md بخش‌های ۶.۵/۶.۶
///             برای مدل کامل فعلی مراجعه کن؛ توضیح قدیمی «۵۰٪ ثابت بلاک‌ریوارد» دیگر صادق
///             نیست. فی تراکنش هرگز اینجا وارد نمی‌شود — ۷۰٪ فی مستقیم و به نسبت بلاک بین
///             ولیدیتورها می‌رود، و ۳۰٪ باقی‌مانده برای همیشه سوزانده می‌شود (بخش ۷) — خزانه
///             از هیچ‌کدام سهمی نمی‌برد.
///           - استیک اسلش‌شده که مستقیم توسط ValidatorsRegistry در هر دموت به‌خاطر غیرفعالی
///             فوروارد می‌شود (به کامنت مستندات «کارمزد عضویت» در ValidatorsRegistry برای
///             مدل ترکیبی استیک مراجعه کن). ✅ خودِ کارمزد عضویت دیگر اینجا نمی‌آید — به
///             BlockRewardDistributor می‌رود، در epoch فی بعدی تاشده و ۱۰۰٪-به‌نسبت-بلاک بین
///             ولیدیتورهای فعال پرداخت می‌شود (sur-tokenomics.md بخش ۶).
///
...
contract ValidatorsTreasury {
    ...
    // ------------------------------------------------------------------
    // دریافت خودکار سورن بومی — ✅ به‌روزشده: از باقی‌مانده‌ی سهم BlockRewardDistributor
    // (بعد از کسر سهم ثابت ۱۵٪ بنیاد و سهم مستقیم حکمرانی‌شونده‌ی ولیدیتورها — دیگر ۵۰٪ ثابت
    // نیست)، و فقط از استیک اسلش‌شده‌ی ValidatorsRegistry (کارمزد عضویت دیگر اینجا نمی‌آید).
    // ------------------------------------------------------------------
